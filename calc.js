...
function controllaFunzioniSpeciali(funzioneCalcolo) {
    ...
    switch (funzioneCalcolo) {
        case "C":
            ...
            termineAttuale = "0";          // Pulisce il termineAttuale corrente
        ...
        case "+/-":
            // Cambio del segno del termine attuale
            procedeConOperazione = false;  // Impedisce di procedere con l'operazione aritmetica
            if (termineAttuale !== "") {  // Controlla se c'è un termine nel termine Attuale
        ...
        case "%":
            ...
            procedeConOperazione = false;  // Impedisce di procedere con l'operazione aritmetica
            if (termineAttuale !== "") {  // Controlla se c'è un termine nel buffer
...
function eseguiOperazione() {
    switch (operazioneCorrente) {
        ...
        case "X":
            ...
            break;
        case "/":
            bufferTermine = termini[1] !== 0 ? (termini[0] / termini[1]).toFixed(10) : "Impossibile dividere per 0, ignorante!";
...
numeriPulsanti.forEach(pulsante => {
    pulsante.addEventListener('click', () => {
        ...
        if (!isDecimale && termineAttuale.charAt(0) === "0") return;
...
pulsantePunto.addEventListener("click", () => {
    // Aggiunge un punto solo se non esiste già e il buffer non è vuoto
